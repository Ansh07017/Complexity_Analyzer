...
export default function Dashboard() {
  ...
  const getCategoryStats = (categoryId: string) => algorithms.filter(alg => alg.category === categoryId).length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {categories.map((category) => {
            ...
            return (
              ...
              >
                <div className="p-4 border rounded-lg overflow-hidden flex flex-col">
                  ...
                  <h3 className="text-lg font-semibold mb-2 break-words whitespace-normal">{category.name}</h3>
                  <p className="text-sm text-muted-foreground mb-4 break-words whitespace-normal">{category.description}</p>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {popularAlgorithms.map((algorithm) => (
                ...
                >
                  <div className="flex flex-col p-4 border border-border rounded-lg hover:bg-accent transition-colors overflow-hidden">
                    <div className="flex items-start mb-2">
                      <div className={`w-10 h-10 ${getCategoryColor(algorithm.category)} rounded-lg flex-shrink-0 flex items-center justify-center mr-4`}>
                        <i className={`${getCategoryIcon(algorithm.category)} text-gray-600`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium break-words whitespace-normal">{algorithm.name}</p>
                        <p className="text-sm text-muted-foreground break-words whitespace-normal">{algorithm.description}</p>
                      </div>
                    </div>
                    ...
                    >
                      <div className="flex flex-col p-4 border border-border rounded-lg hover:bg-accent transition-colors overflow-hidden">
                        ...
                        <div className="flex flex-col items-start mt-2 space-y-1">
                          <ComplexityBadge complexity={algorithm.timeComplexity} />
                          <span className="text-xs text-muted-foreground capitalize break-words whitespace-normal">
